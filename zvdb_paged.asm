; zvdb-z80 - Z80 implementation with paged memory support
...
VECTOR_ALIGN    EQU     256     ; Vectors aligned to 256-byte boundaries
VECTORS_PER_PAGE EQU    64      ; Vectors per 16KB page (16384/256)
MAX_PAGES       EQU     14      ; Maximum pages for vectors (pages 0-13)
MAX_VECTORS     EQU     896     ; Maximum vectors (14 pages × 64 vectors)
...
; Format: [page:1][index:1][count:1][next:1]
hash_index:
        DEFS    256 * 4         ; 1KB
...
; Vector index table (stores page and offset for each vector)
; Format: [page:1][offset:1]
vector_index:
        DEFS    MAX_VECTORS * 2 ; 1792 bytes

; Variables
vector_count:   DEFW    0       ; Current number of vectors (16-bit for >256)
...
switch_vector_page:
        ...
        ; Calculate page number (index / 64)
        LD      A,H
        SRL     A              ; Divide by 256
        SRL     A              ; Divide by 64 (total /= 64)
        LD      D,A            ; D = page number
...
.same_page:
        ; Calculate offset within page (index & 63) * 256
        LD      A,L
        AND     #3F            ; Modulo 64
        LD      H,A
        LD      L,0            ; HL = offset * 256
        LD      DE,PAGE_VECTORS
...
; Both vectors must be in accessible memory
...
; HL = pointer to vector data (in current memory)
add_vector:
        PUSH    HL
        ...
        ; Get vector count and calculate page/offset
        LD      HL,(vector_count)
        PUSH    HL
        ...
        ; Update vector index table
        LD      HL,(vector_count)
        ADD     HL,HL          ; *2 for index table
        LD      DE,vector_index
        ADD     HL,DE
        ...
        LD      A,(vector_count+1)
        SRL     A
        SRL     A              ; Page = high byte >> 2
        LD      (HL),A
        INC     HL
        
        ; Store offset within page
        LD      A,(vector_count)
        AND     #3F            ; Offset = low byte & 63
        LD      (HL),A
...
.bucket_used:
        ...
        ; In full implementation, would chain entries
...
hash_search:
        ...
        ; For now, just check first entry
        ; Full implementation would check all chained entries
        EX      DE,HL          ; HL = vector index
        
        ; Copy vector to buffer and compute score
        LD      DE,temp_vector
        CALL    copy_vector_to_buffer
